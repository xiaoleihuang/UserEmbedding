...
def run_bilstm(data_name, params):
    ...
    if user_emb is not None:
        ...
        gru_unit = gru_unit(embed, initial_state=user_emb_layer)
        gru_unit_b = gru_unit_b(embed, initial_state=user_emb_layer)
    else:
        gru_unit = gru_unit(embed)
        gru_unit_b = gru_unit_b(embed)
        user_input = None

    # dense
    dense_l = Dense(params['hidden_num'], activation='relu', name='dense')(Concatenate(axis=-1)([gru_unit, gru_unit_b]))
    ...
    for e in range(params['epochs']):
        ...
        if f1_valid > best_valid_f1:
            ...
            test_result = open('./results/bilstm_personalize_init{}_results.txt'.format(output_suffix), 'a')
